...
MP4_TO_FRAMES = os.path.join(TOOLS_DIR, "mp4_to_frames.py")
...
def test_basic_extraction():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Extract 4 frames.
        result = run_tool([MP4_TO_FRAMES, video, "4", "--output-dir", frames_dir, "--prefix", "test"])
...
def test_region_crop():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = run_tool([MP4_TO_FRAMES, video, "1", "--output-dir", frames_dir, "--region", "10", "10", "160", "130"])
        assert result.returncode == 0, f"mp4_to_frames failed: {result.stderr}"
...
def test_region_with_width_scaling():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run_tool([GEN_MP4, video, "--width", "320", "--height", "240", "--duration", "1"])
        result = run_tool([MP4_TO_FRAMES, video, "1", "--output-dir", frames_dir,
...
def test_time_range():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = run_tool([MP4_TO_FRAMES, video, "3", "--output-dir", frames_dir, "--start", "1", "--end", "3"])
        assert result.returncode == 0, f"mp4_to_frames failed: {result.stderr}"
...
def test_invalid_region():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run_tool([GEN_MP4, video, "--width", "320", "--height", "240", "--duration", "1"])
        result = run_tool([MP4_TO_FRAMES, video, "1", "--output-dir", frames_dir, "--region", "0", "0", "400", "300"])
...
def test_output_dir_cleanup():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # First run: 4 frames.
        run_tool([MP4_TO_FRAMES, video, "4", "--output-dir", frames_dir])
        assert len(os.listdir(frames_dir)) == 4

        # Second run: 2 frames — old files should be gone.
        run_tool([MP4_TO_FRAMES, video, "2", "--output-dir", frames_dir])
