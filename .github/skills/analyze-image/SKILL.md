---
name: analyze-image
description: Analyze an image and report statistics (dimensions, colors, depth, parallax layer recommendations). Use this skill before split-image-layers to determine optimal layer count.
---
...
## Workflow
Use before `split-image-layers` to decide how many layers:
1. `uv run analyze-image image.png --depth` → get recommended layers
2. `uv run split-image-layers --input image.png --layers N` → split
